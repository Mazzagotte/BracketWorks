...
def _compute_entries_signature(
    db: Session, tournament_id: int, squad_id: Optional[int]
) -> str:
    ...
    for player in sorted(players_query.all(), key=lambda p: p.id):
        raw_entries = (
            player.bracket_entries if isinstance(player.bracket_entries, dict) else {}
        )
        ...
        normalized_entries.setdefault(
            "scratch", max(0, int(player.scratch_entries or 0))
        )
        normalized_entries.setdefault(
            "handicap", max(0, int(player.handicap_entries or 0))
        )
...
def ensure_legacy_bracket_views(bracket_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    bracket_data["scratch_brackets"] = groups_by_key.get("scratch", {}).get(
        "brackets", bracket_data.get("scratch_brackets", [])
    )
    bracket_data["handicap_brackets"] = groups_by_key.get("handicap", {}).get(
        "brackets", bracket_data.get("handicap_brackets", [])
    )
    return bracket_data
...
def save_brackets_simple(
    ...
    try:
        ...
        logger.info("Saving brackets to simple_brackets table")
        ...
        if brackets_data.get("scratch_brackets"):
            ...
            if first_bracket.get("rounds"):
                ...
                logger.debug("  Sample first match being saved:")
                logger.debug(
                    "     %s (scoreA=%s) vs %s (scoreB=%s)",
                    first_match.get("playerA"),
                    first_match.get("scoreA"),
                    first_match.get("playerB"),
                    first_match.get("scoreB"),
                )

        # Preserve existing player_count if not explicitly provided (e.g. match score updates)
        if player_count is None:
            existing_count = (
                db.query(SimpleBracket.player_count)
                .filter(
                    SimpleBracket.tournament_id == tournament_id,
                    (
                        SimpleBracket.squad_id == squad_id
                        if squad_id
                        else SimpleBracket.squad_id.is_(None)
                    ),
                    SimpleBracket.is_active.is_(True),
                )
                .first()
            )
        ...
        if not brackets_data.get("entries_signature"):
            brackets_data["entries_signature"] = _compute_entries_signature(
                db, tournament_id, squad_id
            )
...
def save_first_round_to_history(
    db: Session, tournament_id: int, brackets_data: Dict[str, Any]
) -> None:
    """
...
def load_brackets_simple(
    ...
    """
    try:
        logger.info("Loading brackets from simple_brackets table")
        logger.debug(
            f"  Tournament: {tournament_id}, Squad: {squad_id}, Refresh scores: {refresh_scores}"
        )

        bracket_record = (
            db.query(SimpleBracket)
            .filter(
                SimpleBracket.tournament_id == tournament_id,
                (
                    SimpleBracket.squad_id == squad_id
            ...
            .order_by(SimpleBracket.created_at.desc())
            .first()
        )

        if not bracket_record:
            logger.debug("  No brackets found")
            return None
        ...
        if refresh_scores:
            logger.debug("  Refreshing scores from database...")
            bracket_data = hydrate_brackets_with_scores(
                db, tournament_id, squad_id, bracket_data
            )

        if bracket_data.get("bracket_groups") and bracket_data["bracket_groups"][0].get(
            "brackets"
        ):
            first_bracket = bracket_data["bracket_groups"][0]["brackets"][0]
            if first_bracket.get("rounds"):
                first_match = first_bracket["rounds"][0]["matches"][0]
                logger.debug("  Sample first match being loaded:")
                logger.debug(
                    "     %s (scoreA=%s) vs %s (scoreB=%s)",
                    first_match.get("playerA"),
                    first_match.get("scoreA"),
        ...
        def apply_score_to_match(match, sa, sb, r_idx):
            ...
            if sa > sb:
                ...
                if r_idx == 2:  # Finals (round index 2 = Round 3)
                    ...
                    match["elimination_notes"] = (
                        f"Finals tie - pot split evenly between {match.get('playerA')} and {match.get('playerB')}"
                    )
                else:
                    ...
                    match["split_pot"] = False
                    match["elimination_notes"] = (
                        f"Tied in Round {r_idx + 1} - both players advance, lower Round {r_idx + 2} score will be eliminated"
                    )

        if bracket_type == "rounds":
            # Single bracket format
            if round_index < len(bracket_data["rounds"]) and match_index < len(
                bracket_data["rounds"][round_index]["matches"]
            ):
            ...
            group = next(
                (
                    item
                    for item in bracket_data.get("bracket_groups", [])
                    if item.get("key") == bracket_type
                ),
                None,
            )
            if (
                group
                and bracket_index is not None
                and bracket_index < len(group.get("brackets", []))
                and "rounds" in group["brackets"][bracket_index]
                and round_index < len(group["brackets"][bracket_index]["rounds"])
                and match_index
                < len(
                    group["brackets"][bracket_index]["rounds"][round_index]["matches"]
                )
            ):

                match = group["brackets"][bracket_index]["rounds"][round_index][
                    "matches"
                ][match_index]
                apply_score_to_match(match, score_a, score_b, round_index)
        else:
            ...
            if (
                bracket_type in bracket_data
                and bracket_index is not None
                and bracket_index < len(bracket_data[bracket_type])
                and "rounds" in bracket_data[bracket_type][bracket_index]
                and round_index
                < len(bracket_data[bracket_type][bracket_index]["rounds"])
                and match_index
                < len(
                    bracket_data[bracket_type][bracket_index]["rounds"][round_index][
                        "matches"
                    ]
                )
            ):

                match = bracket_data[bracket_type][bracket_index]["rounds"][
                    round_index
                ]["matches"][match_index]
                apply_score_to_match(match, score_a, score_b, round_index)
...
def hydrate_brackets_with_scores(
    ...
    for bowler_id in list(scores_map_total.keys())[:3]:
        ...
        logger.debug(
            "    Bowler %s: Scratch=(G1=%s, G2=%s, G3=%s), Total=(G1=%s, G2=%s, G3=%s)",
            bowler_id,
            scratch.get("game1"),
            scratch.get("game2"),
            scratch.get("game3"),
    ...
    def update_match_scores(
        ...
        if old_score_a != score_a or old_score_b != score_b:
            logger.debug(
                f"    Updated match: {match.get('playerA')} vs {match.get('playerB')}: {old_score_a}->{score_a}, {old_score_b}->{score_b}"
            )
        ...
        if score_a is not None and score_b is not None:
            if score_a > score_b:
                ...
                if round_num == 2:  # Round 3 is index 2
                    ...
                    logger.info(
                        f"  Round 3 tie detected: {match.get('playerA')} vs {match.get('playerB')} - pot split"
                    )
                # Round 1 or Round 2 - both advance
                    ...
                    match["split_pot"] = False
                    match["elimination_notes"] = (
                        f"Tied in Round {round_num + 1} - both players advance, lower Round {round_num + 2} score will be eliminated"
                    )
                    logger.info(
                        f"  Round {round_num + 1} tie detected: {match.get('playerA')} vs {match.get('playerB')} (both {score_a}) - both advance"
                    )
    ...
    for group, _, bracket in iter_group_brackets(bracket_data):
        ...
        for round_num, round_data in enumerate(bracket.get("rounds", [])):
            for match in round_data.get("matches", []):
                update_match_scores(
                    match, round_num, use_scratch=use_scratch, use_reverse=use_reverse
                )
                matches_updated += 1
    ...
    def propagate_and_rehydrate(
        bracket: Dict, use_scratch: bool, use_reverse: bool = False
    ):
        rounds = bracket.get("rounds", [])
        for round_idx in range(len(rounds) - 1):
            ...
            for match_idx, match in enumerate(current_matches):
                ...
                winner_name = (
                    match.get("playerA") if winner == "A" else match.get("playerB")
                )
                winner_id = (
                    match.get("playerA_id")
                    if winner == "A"
                    else match.get("playerB_id")
                )
            ...
            if changed:
                ...
                for match in next_matches:
                    update_match_scores(
                        match, round_idx + 1, use_scratch, use_reverse=use_reverse
                    )
    ...
    def get_player_score_for_round(
        player_id: Optional[int], round_idx: int, use_scratch: bool, use_reverse: bool
    ) -> Optional[int]:
    ...
    def resolve_tie_using_next_round(
        tied_match,
        next_round_idx: int,
        final_round_idx: int,
        use_scratch: bool,
        ...
        score_a = get_player_score_for_round(
            pid_a, next_round_idx, use_scratch, use_reverse
        )
        score_b = get_player_score_for_round(
            pid_b, next_round_idx, use_scratch, use_reverse
        )
        ...
        if score_a > score_b:
            ...
            logger.info(
                f"  Resolved tie: {tied_match.get('playerA')} wins ({score_a} > {score_b} in Round {next_round_num})"
            )
            ...
            logger.info(
                f"  Resolved tie: {tied_match.get('playerB')} wins ({score_b} > {score_a} in Round {next_round_num})"
            )
            ...
            if next_round_idx >= final_round_idx:
                ...
                logger.info(
                    f"  Tie persists through finals: {tied_match.get('playerA')} vs {tied_match.get('playerB')} — pot split"
                )
            else:
                ...
                logger.info(
                    f"  Tie cascades to Round {next_round_num + 1}: {tied_match.get('playerA')} vs {tied_match.get('playerB')}"
                )
    ...
    for group, _, bracket in iter_group_brackets(bracket_data):
        ...
        for round_idx in range(total_rounds - 1):
            ...
            for match in current_matches:
                if match.get("both_advance") and match.get("scoreA") == match.get(
                    "scoreB"
                ):
...
def delete_brackets_simple(
    db: Session, tournament_id: int, squad_id: Optional[int] = None
) -> int:
    """

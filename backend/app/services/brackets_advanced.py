...
def is_forbidden(
    player_a_id: int, player_b_id: int, history_set: Set[Tuple[int, int]]
) -> bool:
    """
...
def max_fill_groups(
    entries: List[Dict[str, Any]], bracket_size: int, rng: random.Random
...
def optimize_pairings(
    ...
    bye_recipient_counts: Dict[int, int] = (
        {}
    )  # Avoid repeatedly giving BYE to same player

    for group in groups:
        best_pairings: Optional[
            List[Tuple[Dict, Dict, Tuple[int, int], Optional[int]]]
        ] = None
        ...
        if best_pairings is not None:
            for _, _, p, bye_recipient_id in best_pairings:
                if p != (-1, -1):
                    ...
                    bye_recipient_counts[bye_recipient_id] = (
                        bye_recipient_counts.get(bye_recipient_id, 0) + 1
                    )
            brackets.append(
                {
                    "entrants": group,
                    "pairings": [
                        {"home": a, "away": b} for a, b, _, _ in best_pairings
                    ],
                }
            )
        else:
...
def create_single_bracket_from_pairings(
    pairings: List[Dict[str, Any]], title: str, bracket_size: int
) -> Dict[str, Any]:
    """
    ...
    for i, pairing in enumerate(pairings):
        ...
        logger.debug(
            f"  Match {i+1}: {home_player.get('name')} vs {away_player.get('name')}"
        )
        ...
        logger.debug(
            f"    Created match with scoreA={score_a}, scoreB={score_b}, winner={winner}, status={status}"
        )
    ...
    while len(current_matches) > 0:
        ...
        rounds.append({"name": round_name, "matches": current_matches.copy()})
...
def generate_brackets_with_constraints(
    ...
    **kwargs,  # Accept (and ignore) legacy keyword args for backwards compat
    ...
    assert is_power_of_two(
        bracket_size
    ), f"Bracket size must be power of 2, got {bracket_size}"
    ...
    return {"brackets": brackets, "refunded": refunded}
...
def create_brackets_with_history(
    ...
    result = generate_brackets_with_constraints(
        entries=entries, bracket_size=bracket_size, history_set=history_set, seed=seed
    )
    ...
    for i, bracket_data in enumerate(result["brackets"]):
        ...
        bracket = create_single_bracket_from_pairings(
            bracket_data["pairings"], bracket_title, bracket_size
        )
    ...
    if allow_single_bye_per_bracket and result.get("refunded"):
        ...
        if real_slots > 0:
            ...
            for bye_count in range(1, max_bye_brackets + 1):
                ...
                candidate_rng = (
                    random.Random(seed + bye_count)
                    if seed is not None
                    else random.Random()
                )
                capacities = [bracket_size for _ in range(normal_count)] + [
                    real_slots for _ in range(bye_count)
                ]
                ...
                for player_entries in sorted_players:
                    ...
                    assignable_entries = fisher_yates_shuffle(
                        player_entries[:target_groups], candidate_rng
                    )
                    ...
                    for entry in assignable_entries:
                        eligible = [
                            ...
                            and all(
                                existing.get("player_id") != player_id
                ...
                if candidate_placed > best_layout_placed or (
                    candidate_placed == best_layout_placed
                    and normal_count > best_layout_normal_count
                ):
            ...
            while True:
                ...
                greedy_group: List[Dict[str, Any]] = []
                for pid in selected:
                    greedy_group.append(by_player_for_greedy[pid].pop())

                greedy_groups.append(greedy_group + [bye_entry.copy()])

            if greedy_groups:
                candidate_rng = (
                    random.Random((seed or 0) + 99991)
                    if seed is not None
                    else random.Random()
                )
                optimized_brackets, failed_groups = optimize_pairings(
            ...
            if (
                best_layout_brackets is not None
                and best_layout_refunded is not None
                and best_layout_placed > standard_placed
            ):
        ...
        leftover_players.append(
            {
                ...
                "reason": "Could not create valid pairing (constraints or insufficient players)",

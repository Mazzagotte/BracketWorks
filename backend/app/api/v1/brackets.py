...
from ...services.brackets import generate_bracket_preview, generate_multiple_brackets, update_match_score
from ...services.bracket_persistence import (
    save_generated_brackets, 
    load_generated_brackets, 
    delete_existing_brackets,
    bracket_exists
)
...
def update_match_score_endpoint(
    ...
    try:
        ...
        result = generate_tournament_brackets(tournament_id, squad_id, False, db)
        ...
        try:
            save_generated_brackets(db, tournament_id, squad_id, updated_result)
...
def generate_tournament_brackets(
    ...
    try:
        ...
        if not force_regenerate:
            ...
            # Check database
            if bracket_exists(db, tournament_id, squad_id):
                existing_brackets = load_generated_brackets(db, tournament_id, squad_id)
        ...
        # Generate brackets
        brackets_result = generate_multiple_brackets(players_data, bracket_settings.bracket_size)
        
        # Save the generated brackets to database
        try:
            save_generated_brackets(db, tournament_id, squad_id, brackets_result)
        except Exception as save_error:
...
def delete_tournament_brackets(
    ...
    try:
        ...
        if not bracket_exists(db, tournament_id, squad_id):
...
def check_brackets_exist(
    ...
    try:
        exists = bracket_exists(db, tournament_id, squad_id)
